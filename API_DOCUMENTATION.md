...
4. [Eventos](#eventos)
5. [Sponsors](#sponsors)
6. [Enlaces de Invitación de Sponsors](#enlaces-de-invitación-de-sponsors)
7. [Invitaciones](#invitaciones)
8. [Staff Operativo](#staff-operativo)
9. [Participantes](#participantes)
10. [Tickets](#tickets)
11. [Códigos QR](#códigos-qr)
12. [Flujos de Integración](#flujos-de-integración)
...
```json
{
  ...
  "guestQuota": 10,
  "scholarshipQuota": 3,
  "benefits": ["Logo en material promocional", "Stand premium de 20m²"]
}
```

**Response:** `201 Created`

### Actualizar Cuotas de Sponsor

**Endpoint:** `PATCH /events/:eventId/sponsors/:sponsorId`
...
### Eliminar Sponsor de Evento

**Endpoint:** `DELETE /events/:eventId/sponsors/:sponsorId`
...
## 🔗 Enlaces de Invitación de Sponsors
...
- `maxUses`: Requerido si `usageType` es "multiple" (mínimo 1)
- `ticketTypeId`: Opcional - ID del tipo de ticket a asignar
...
### Listar Enlaces de Invitación de un Evento

**Endpoint:** `GET /events/:eventId/invitations`

**Headers:** `Authorization: Bearer {token}`

**Roles:** PLATFORM_ADMIN, COMPANY_ADMIN, STAFF

**Query Params:**

- `sponsorId` (opcional): Filtrar por sponsor específico
- `participantType` (opcional): Filtrar por tipo (staff, guest, scholarship, regular, operational_staff)
...
**Endpoint:** `PATCH /events/:eventId/sponsors/:sponsorId/invitations/:invitationId/deactivate`
...
const staffResponse = await fetch(
  "http://localhost:3000/users/me/staff-roles",
  {
...
const validateResponse = await fetch(
  `http://localhost:3000/invitations/ABC123XYZ/validate`
);
...
if (validation.isValid) {
  ...
  const acceptResponse = await fetch(
    "http://localhost:3000/invitations/ABC123XYZ/accept",
    {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
...
console.log(
  "Link de invitación:",
  `https://tu-app.com/invitations/${invitation.code}`
);
```
